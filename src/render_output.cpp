...
void RenderOutput::createPostDescriptor()
{
  ...
  for (int i = 0; i < 2; i++) {
      ...
      writes.emplace_back(bind.makeWrite(m_postDescSet[i], OutputBindings::eIndirectSampler, &m_indirectResult[i].descriptor));
      writes.emplace_back(bind.makeWrite(m_postDescSet[i], OutputBindings::eDirectResult, &m_directResult[i].descriptor));
      writes.emplace_back(bind.makeWrite(m_postDescSet[i], OutputBindings::eIndirectResult, &m_indirectResult[i].descriptor));
